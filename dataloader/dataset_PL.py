...
class PLY_dataset(data.Dataset):
    def __init__(self, data_path, sample_interval, time_step, label_convert_fun=None, return_ref=False, crop_data=None):
        ...
        self.xyz = np.array(
            np.transpose(np.stack((plydata['vertex']['x'], plydata['vertex']['y'], plydata['vertex']['z'])))).astype(
            np.float32)
        try:
    ...
    def __getitem__(self, index):
        ...
        if self.crop_data:
            data_indexes = random.sample(data_indexes, k=self.crop_data)
...
class2coarse = np.array([[int(i.attributes['id'].value), int(i.attributes['coarse'].value)] for i in itemlist]).astype(
    np.uint32)
coarseID_name = {int(i.attributes['coarse'].value): i.attributes['coarse_name'].value for i in itemlist}
for i in range(len(coarseID_name) - 1):
    coarseID_name[i] = coarseID_name.pop(i + 1)
