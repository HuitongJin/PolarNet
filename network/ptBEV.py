...
class ptBEVnet(nn.Module):
    def __init__(self, BEV_net, grid_size, pt_model='pointnet', fea_dim=3, pt_pooling='max', kernel_size=3,
        ...
        if kernel_size != 1:
            if self.pt_pooling == 'max':
                self.local_pool_op = torch.nn.MaxPool2d(kernel_size, stride=1, padding=(kernel_size - 1) // 2,
                                                        dilation=1)
            else:
